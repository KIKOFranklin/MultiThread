 ...
 *          需要注意的是 当其中一个线程先获取到写锁时 读锁会同时被阻塞 写锁中的unlock也只是唤醒其他想获取写锁的线程来竞争写锁 只有当获取写锁的线程全部死亡后 读锁才能够被获取到
 *          反之 当其中一个线程先获取到读锁时 写锁也会同时被阻塞 只有当读锁完全释放时 写锁才能够被获取到 读锁是共享锁 线程之间可以一起使用
 *          需要再次强调的是 虽然有读锁这个概念 但是线程执行仍是并发执行的 并没有出现等待的状况 而写锁则类似于synchronized 是一把重量级锁 同样也是一把独占锁
 *
