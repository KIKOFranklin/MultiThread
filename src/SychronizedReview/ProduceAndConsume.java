...
/**
 * @author KIKOFranklin
 * @since 2021/4/15 0015 14:36
 *
 * wait()使当前线程阻塞，
 *
 * 前提是必须先获得锁
 *
 * 一般配合synchronized关键字使用-->在synchronized同步代码块里使用 wait()、notify/notifyAll()方法。
 *
 * 由于 wait()、notify/notifyAll() 在synchronized 代码块执行，说明当前线程一定是获取了锁的。
 *
 * 当线程执行wait()方法时候，会释放当前的锁，然后让出CPU时间片，进入等待状态。
 *
 * 只有当 notify/notifyAll() 被执行时候，才会唤醒一个或多个正处于等待状态的线程，然后继续往下执行，直到执行完synchronized代码块的代码或是中途遇到wait() ，再次释放锁。
 *
 ...
 *
 * notifyAll 会唤醒所有等待(对象的)线程，哪一个线程第一个获取锁取决于操作系统的实现。如果当前情况下有多个线程需要被唤醒，推荐使用notifyAll 方法。
 *
